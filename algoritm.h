...
int algoritm(unsigned __int64 oll_mask) {
	std::cout << "start\n";
	...
	int pos;
	bool ind;
	ifstream fin1("u1.txt");
	ifstream fin2("u2.txt");
	ifstream fin3("u3.txt");
	...
	ifstream finz1("uz1.txt");
	ifstream finz2("uz2.txt");
	ifstream finz3("uz3.txt");

	ifstream fin_nav("navigator.txt");
	if (!fin1.is_open() || !fin2.is_open() || !fin3.is_open() || !finz1.is_open() || !finz2.is_open() ||
		!finz3.is_open() || !fingamma.is_open())
	{
		cout << "Some of files did not open!\n";

		return 1;
	}
	//заполнение матрицы изначальных данных свободный член не вводим
	...
	//создание структур для редуцирования свободные члены в наборах мж уже пропущены
...
//101101000010011001 1101110000000110010111 1100101100101011100001 
	...
	/*mask_1 = (oll_mask & 0x7FFFF);
	mask_2 = ((oll_mask >> 19) & 0x3FFFFF);
	mask_3 = ((oll_mask >> 41) & 0x7FFFFF);*/
	for (int i = 0; i < 276; i++) {
		fin_nav >> navigator[i];

	}
	fin_nav.close();
	pos_1 = ((pos_1 | oll_mask) & 0x7FFFF);
	pos_2 = ((pos_2 | (oll_mask >> 19)) & 0x3FFFFF);
	pos_3 = ((pos_3 | (oll_mask >> 41)) & 0x7FFFFF);
	printf("pos_1: %.4X\n", pos_1);
	printf("pos_2: %.4X\n", pos_2);
	printf("pos_3: %.4X\n", pos_3);
	// тут нужно сделать пребор значений


	//подсчет количества перебираемых битов
	for (int p = 0; p < 19; p++) {
		if (((pos_1 >> p) & 1)) {
			count_val_1++;
		}
	...
	//генерация очередных перебираемых значений
	for (int i_1 = 0; i_1 <= max_val_1; i_1++) {
		...
		for (int i_2 = 0; i_2 <= max_val_2; i_2++) {
			...
			for (int i_3 = 0; i_3 <= max_val_3; i_3++) {
				...
				cout << "val: " << val_1 << " " << val_2 << " " << val_3 << endl;
				//чистим и заполняем систему
				...
				////редуцирование исходных данных 190 253 276  внимание! скобки при логических операциях
				///111 отлажен
				mask_1 = 0;
				for (int p = 0; p < 19; p++) {
					if ((pos_1 & (1 << p)) != 0) {
				...
				//редуцировали остаётся внести в сиситему (не забываем про свободный член, и используя суммы масок xor) СВОБОДНЫЙ ЧЛЕН В 719
				//считаем итоговый размер. выкидываем лишние мономы
				final_size_1 = 19;
				for (int p = 0; p < 19; p++) {
					if ((pos_1 & (1 << p)) != 0) {
						final_size_1--;
				...
				//сдвиги относительно остальных систем
				////22222
				...
				//сборка уравнений 0x1ffff 32768 65536 98304 131072
				for (key = 0; key <= 0x1ffff; key++) {//0x1ffff
					...
					if (ind) {
						...
						//восстанавливаем значение регистров после инициализации
						...
						for (int i = 0; i < 190; i++) {
							if (!(navigator[i] & pos_1)) {
								...
								if (count == 1) {                          //101101000010011001 
										//std::cout << (*it)[final_size]; //1100100001011010
								...
								if (count == 1) {                          //101101000010011001 
										//std::cout << (*it)[final_size]; //1100100001011010   10011001000010110100
						...
						//std::cout << endl << temp_reg_1 << " " << temp_reg_2 << " " << temp_reg_3 << endl;
						...
						ifstream ink1("outkey1.txt");
						ifstream ink2("outkey2.txt");
						ifstream ink3("outkey3.txt");
						ifstream ink4("outkey4.txt");
						if (!ink1.is_open() || !ink2.is_open() || !ink3.is_open() || !ink4.is_open())
						{
							cout << "Some of files did not open!\n";
